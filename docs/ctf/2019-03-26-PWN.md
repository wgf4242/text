...
6.[x64程序](#level2_x64) 32位是通过栈传参，而64位通过edi寄存器传参，需要覆盖edi的值
...
x64程序 -- 通过rdi, rsi, rdx, rcx, r8, r9中 传参, retn前要pop rdi, payload: junk, pop_rdi_ret, bin_sh, system

Ubuntu版本大于等于18后调用system函数需要考虑栈平衡, level2填充a*88后发个ret, 再+其他payload
