...
print(f"处理器名称是：{idaapi.inf_get_procname()}")
...
print(f"suspect limits are: {idaapi.inf_get_lowoff():x}, {idaapi.inf_get_highoff():x}")

print(f"string prefix is {idaapi.inf_get_strlit_pref()}")
print(f"string type is {idaapi.inf_get_strtype()}")
...
print(f"Notepad text is <begin>{idaapi.get_ida_notepad_text()}</end>")  # view - Open Subview - Notepad
print(f"creation time is {idaapi.get_idb_ctime()}")
