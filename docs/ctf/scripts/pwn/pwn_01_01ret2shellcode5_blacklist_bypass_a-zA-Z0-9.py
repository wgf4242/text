"""NewstaCTF2023 shellcode revenge [a-zA-Z0-9]
...
LABEL_9:
  ...
  通过xor的操作对寄存器赋值，题目已经预先设置好read的寄存器，只需要执行出syscall就好，syscall的read不限制字符串输入，但是输入的地址是在rsi=0x66660000，目前执行的地址是有一定偏移的，加入一些nop滑板然后直接使用pwntools生成就好。可以参考b站imLZH1师傅的视频BV1Z14y1B7ji
"""
from pwn import *
