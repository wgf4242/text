...
3.   如果提示monty要100000可以使用money[]=100000
...
0x4 MIME绕过: 
    ...
    bp抓包 修改 Content-Type: application/octet-stream 为 Content-Type: image/jpg
...
常用方法
    ...
    print_r, vardump(array), 把array 转字符串
...
Proxy-Option-Intercept Client Requests, 添加does not match [url]
    ...
    ^.*(baidu|g-fox|firefox|mozilla|googleapi|alicdn).*$
