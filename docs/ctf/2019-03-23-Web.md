...
```
    ...
    fuzz_list-regexbuddy测: 
        mysql -- 见sql_fuzz.json --  /**/|sElect|uNion|.|oR|aNd|	|"|'|^|+|Hex|0x|0b1111101000|?|sHow|dEsc|orDer By|--|~~|`|CONcat|aLter|Columns|*|UpdateXml|ExtractValue|load_file
                 MySQL 遇到 hex 会自动转成字符串 , 0x61 即 a
                 只能是数字: '0' + 使用2次hex + '0' 见下面 '' 
        ...
        Linux -- /dir/ls|bash|tac|nl|more|less|head|wget|tail|vi|cat|od|grep|sed|bzmore|bzless|pcre|paste|diff|file|echo|sh|\'|\"|\`|;|,|\*|\?|\\|\\\\|\n|\t|\r|\xA0|\{|\}|\(|\)|\&[^\d]|@|\||\\$|\[|\]|{|}|\(|\)|-|<|>/i
...
```
### 通过replace bypass
第五空间 yet_another_mysql_injection
