...
3. str_replace('../', '')   双写POC:  ..././ -> ../
4. 查看html隐藏字段提交
1. 查看 robots.txt, hint.txt, index.php, flag.php, index.php~, 当前页 php
...
10. php 相关
    ...
    6. phar反序列化漏洞
    7. preg_replace() /e 代码执行漏洞, 与双引号执行
    8. 查看rce章节
    9. ssti注入 Twig: {{7*'7'}} 输出49  [BJDCTF2020]Cookie is so stable https://www.cnblogs.com/wkzb/p/12422190.html
...
```
1^(ascii(substr((select(group_concat(password))from(F1naI1y)),%d,1))>%d)" %(i,d)# i d是参数
或
