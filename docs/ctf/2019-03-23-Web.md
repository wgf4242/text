...
```
    ...
    fuzz_list-regexbuddy测: 
        mysql -- /**/|sElect|uNion|.|or|"|'|^|0x|0b1111101000|?|sHow|dEsc|orDer By|--|~~|`|CONcat|aLter|Columns|*|UpdateXml|ExtractValue|load_file
        php函数-- print_r(glob("*"))|
...
if (md5_file($receiveFile) === md5_file("key.dat")) {   # 多进程并发请求，只要满足第一个条件后(先发key.dat)，修改文件内容第二个条件即可成功
