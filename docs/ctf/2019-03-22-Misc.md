...
10. 图片隐写 
    ...
    -- GIF文件 
      ...
      ---- stegsolve 按帧看
      ---- 看帧数转01 NCTF2018-have fun , ,  `identify -format "%T \n" filename`
    -- stegsolve 看各通道, 看lsb隐写
      ---- 0通道提取
    -- 盲水印：双图，python3 bwmforpy3.py decode 00001537.png 00001404.jpg flag.png --alpha 10
    -- 盲水印：单图 imagin 或 java -jar BlindWatermark.jar decode -c ec.jpg output.jpg
    ...
    -- outguess -k 密码 -r 需要破解的文件名 -t 转出的文件名
...
11. 音频隐写 https://www.sqlsec.com/2018/01/ctfwav.html
    -- 1.Audition/Audacity看 多是摩斯码, 2.看频谱spectrogram(视图-频谱) 3.听歌 4.misc_dtmf.zip
    -- 效果-反向 听声音。
    -- 删除多余文件头，可能有2段riff
    ...
    -- SilentEye
    -- SSTV扫描, 频谱图比较平均 见 ### sstv

11. 文字隐写 
    -- snow可Google:space tab steg, baidu: 空格 制表符 隐写
    -- snow.exe -C flag.txt
    -- base64隐写

12. ascii图形用终端  cat 来看。
...
11. doc文件/office 1. 显示隐藏文字 2. 解压看有没有隐藏图片
...
2.用mimicat或者 使用Passware Kit 13来破解Yusa用户的密码
